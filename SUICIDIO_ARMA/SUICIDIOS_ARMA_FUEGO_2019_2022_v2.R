...
setwd("C:/Users/iosoriod/Desktop/ARCHIVOS_2023/C5_2023/SUICIDIO_ARMA/")
#CARGAR BASES-------------------------------------
...
homi<-read.csv("folios_intento_suicidio_disparo_2019_2022_A.csv", fileEncoding = "Latin1", check.names = F)
#COV<-read.csv("COV_ESCUELAS_EDUCACION.csv", header = T,stringsAsFactors = F)

unique(homi$codigo_cierre)
homi<-homi[which((homi$codigo_cierre)=="A"),]
...
title <- tags$div(
  tag.map.title, HTML(paste("SUICIDIO POR ARMA DE FUEGO"))
)
...
colors <- c("#7F7F7F","#17BECF","red")
labels <- c("ALCALDÍAS","COLONIAS",
            paste("SUICIDIO POR ARMA DE FUEGO","(",nrow(homi),")"))
sizes <- c(10,10,10)
shapes <- c("square","square","circle")
borders <- c("#7F7F7F","#17BECF","red")
...
rr <- tags$div(
HTML('<a href="https://i.ibb.co/8g1BXyN/tabla.jpg"> <img border="0" alt="ImageTitle" top= "-50px" src="https://i.ibb.co/8g1BXyN/tabla.jpg" width="334" height="296"> </a>')
)
...
leaflet()  %>%addTiles() %>%addProviderTiles(providers$Esri.WorldGrayCanvas) %>% 
 ...
 addControl(rr,position = "bottomleft")%>% #TABLA DE DATOS
  ...
  addResetMapButton()%>%
  setView(lng = mean(homi$longitud),lat = mean(homi$latitud),zoom = 12) %>%
 ...
 #  
  ...
  # 
  addCircles(data = homi,lng = homi$longitud,lat = homi$latitud,color = "red" ,fillColor = "red",radius = 8,fillOpacity = T,
           popup = paste("<b>","FOLIO : ","</b>",as.character(homi$folio),"<br>",
                         "<b>","TIPO : ","</b>",as.character(homi$incidente_c4)),
           group = paste("SUICIDIO POR ARMA DE FUEGO","(",nrow(homi),")"),options = pathOptions(pane="li"))%>%
...
#ETIQUETAS----------------------------------------------------------
  ...
  addMarkers(data = homi,lng=homi$longitud,lat = homi$latitud,label = homi$folio,group = "TOT")%>%
