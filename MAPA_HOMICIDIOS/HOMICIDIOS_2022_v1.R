...
homi_dolso_otras <-homi[which((homi$`HOMOLOGACIÓN_TIPO DE EVENTO`)=="HALLAZGO DE CADÁVER"),]
hallazgo_cadaver <-homi[which((homi$`HOMOLOGACIÓN_TIPO DE EVENTO`)=="HOMICIDIO DOLOSO POR OTRAS CAUSAS"),]
...
colors <- c("#7F7F7F","#17BECF","red","blue","black","magenta","#FF7F0E","#358000","#9467BD","#8C564B")
labels <- c(
            "ALCALDÍAS","COLONIAS",
            paste("9m","(",nrow(b9t),")"),
            paste("20m","(",nrow(b20t),")"),
            ...
            paste("HOMICIDIO DOLOSO POR OTRAS CAUSAS","(",nrow(hallazgo_cadaver),")"),
            paste("HOMICIDIO DOLOSO POR GOLPES","(",nrow(homi_doloso_golpes),")"),
            paste("HOMICIDIO POR ARMA BLANCA","(",nrow(homi_doloso_golpes),")"))
sizes <- c(10,10,10,10,10,10,10,10,10,10)
shapes <- c("square","square","circle","circle","circle","circle","circle","circle","circle","circle")
borders <- c("#7F7F7F","#17BECF","red","blue","black","magenta","#FF7F0E","#358000","#9467BD","#8C564B")
...
#  addControl(rr,position = "bottomleft")%>% #TABLA DE DATOS
 ...
 addPolygons(data=colonias,color = "#17BECF",fillColor = "#F4A460",fillOpacity =0.005,weight = 3,popup = paste("<b>","COLONIA : ","</b>",colonias$NOM_ASENTA) ,
  ...
  #STVS-------------------------------  
  addCircles(data = arma_blanca,lng = arma_blanca$`LONGITUD X`,lat = arma_blanca$`LATITUD Y`,color = "magenta" ,fillColor = "magenta",radius = 8,fillOpacity = T,
           popup = paste("<b>","FOLIO : ","</b>",as.character(arma_blanca$FOLIO),"<br>",
                         "<b>","TIPO DE POSTE : ","</b>",as.character(arma_blanca$`HOMOLOGACIÓN_TIPO DE EVENTO`)),
  ...
  addCircles(data = arma_fuego,lng = arma_fuego$`LONGITUD X`,lat = arma_fuego$`LATITUD Y`,color = "#FF7F0E" ,fillColor = "#FF7F0E",radius = 8,fillOpacity = T,
           popup = paste("<b>","FOLIO : ","</b>",as.character(arma_fuego$FOLIO),"<br>",
                         "<b>","TIPO DE POSTE : ","</b>",as.character(arma_fuego$`HOMOLOGACIÓN_TIPO DE EVENTO`)),
           group = paste("HOMICIDIO POR ARMA DE FUEGO","(",nrow(arma_fuego),")"),options = pathOptions(pane="li"))%>%
  
  addCircles(data = hallazgo_cadaver,lng = hallazgo_cadaver$`LONGITUD X`,lat = hallazgo_cadaver$`LATITUD Y`,color = "#358000" ,fillColor = "#358000",radius = 8,fillOpacity = T,
           popup = paste("<b>","FOLIO : ","</b>",as.character(hallazgo_cadaver$FOLIO),"<br>",
                         "<b>","TIPO DE POSTE : ","</b>",as.character(hallazgo_cadaver$`HOMOLOGACIÓN_TIPO DE EVENTO`)),
           group = paste("HOMICIDIO DOLOSO POR OTRAS CAUSAS","(",nrow(hallazgo_cadaver),")"),options = pathOptions(pane="li"))%>%
  ...
  addCircles(data = homi_doloso_golpes,lng = homi_doloso_golpes$`LONGITUD X`,lat = homi_doloso_golpes$`LATITUD Y`,color = "#9467BD" ,fillColor = "#9467BD",radius = 8,fillOpacity = T,
           popup = paste("<b>","FOLIO : ","</b>",as.character(homi_doloso_golpes$FOLIO),"<br>",
                         "<b>","TIPO DE POSTE : ","</b>",as.character(homi_doloso_golpes$`HOMOLOGACIÓN_TIPO DE EVENTO`)),
           group = paste("HOMICIDIO DOLOSO POR GOLPES","(",nrow(homi_doloso_golpes),")"),options = pathOptions(pane="li"))%>%
  
  addCircles(data = homi_doloso_golpes,lng = homi_doloso_golpes$`LONGITUD X`,lat = homi_doloso_golpes$`LATITUD Y`,color = "#8C564B" ,fillColor = "#8C564B",radius = 8,fillOpacity = T,
           popup = paste("<b>","FOLIO : ","</b>",as.character(homi_doloso_golpes$FOLIO),"<br>",
                         "<b>","TIPO DE POSTE : ","</b>",as.character(homi_doloso_golpes$`HOMOLOGACIÓN_TIPO DE EVENTO`)),
           group = paste("HOMICIDIO POR ARMA BLANCA","(",nrow(homi_doloso_golpes),")"),options = pathOptions(pane="li"))%>%
