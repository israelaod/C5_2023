...
base<- read.csv("SECTOR_XOTEPINGO.csv", header = T,stringsAsFactors = F, fileEncoding = "Latin1", check.names = F)
...
SECTORES <- spTransform(shapefile("COORDINACIONES_TERRITORIALES_region.shp"), "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")
...
XOTEPINGO<-subset(SECTORES, SECTORES$TERRITORIA=="XOTEPINGO")
...
title <- tags$div(
  tag.map.title, HTML(paste("COBERTURA DE CÁMARAS SECTOR XOTEPINGO "))
)
...
leaflet()  %>%addTiles() %>%addProviderTiles(providers$Esri.WorldGrayCanvas) %>% 
 # addProviderTiles(providers$OpenStreetMap.Mapnik) %>%
  ...
  addResetMapButton()%>%
  setView(lng = mean(base$LONGITUD),lat = mean(base$LATITUD),zoom = 14) %>%
  ...
  # 
  addPolygons(data=XOTEPINGO,color = "GRAY",opacity = 0.9,fillColor = "transparent",fillOpacity =0.01,weight = 3,popup = paste("<b>","SECTOR : ","</b>",XOTEPINGO$TERRITORIA),
              label = XOTEPINGO$TERRITORIA,labelOptions = labelOptions(noHide = T, textOnly = TRUE,direction = "center"),
              highlightOptions = highlightOptions(color = "GRAY", weight = 3) , group="SECTOR XOTEPINGO",options = pathOptions(pane="polygons"))%>%
